...
class CustomerCore extends ObjectModel
{
    ...
    /** @var string Object last time user make a payment date */
    ...
    public function dont_reach_limit_payment_per_day()
    {
      $date_last_payment = $this->date_last_payment;
      if ($date_last_payment != date('Y-m-d')){
        $this->number_payment_day = 0;
        Db::getInstance()->execute('UPDATE `'._DB_PREFIX_.'customer` SET `date_last_payment` = "'.date('Y-m-d').'" WHERE `id_customer` = '.(int)$this->id);
        return true;
      }
      $number_payment = $this->number_payment_day;
      if ($number_payment >= 3) {
        return false;
      }
      return true;
    }
    public function update_payment_time()
    {
        $number_payment = $this->number_payment_day;
        $new_number_payment = $number_payment + 1;
        Db::getInstance()->execute('UPDATE `'._DB_PREFIX_.'customer` SET `number_payment_day` = '.(int)$new_number_payment.' WHERE `id_customer` = '.(int)$this->id);
        Db::getInstance()->execute('UPDATE `'._DB_PREFIX_.'customer` SET `date_last_payment` = "'.date('Y-m-d').'" WHERE `id_customer` = '.(int)$this->id);
    }
