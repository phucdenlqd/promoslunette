    ...
    public function initContent()
    {
        ...
        if (!$this->errors) {
            ...
            $this->context->smarty->assign(array(
               ...
               $cat = new Category($this->category->id);
                                                
		if($cat->level_depth==3 and $cat->id_parent==32  or $cat->level_depth==2 and $cat->id==32  )
		$type='LV';/*lunette de vue */
		else
		$type='NLV';/*Non lunette de vue */
		$this->context->smarty->assign('type' ,$type);
        if(isset($_GET['type'])and $_GET['type']="verre" )
		{
                    

		$id_product = (int)Tools::getValue('id_product');
			 if ($this->context->customer->isLogged()) { 
					$this->setTemplate(_PS_THEME_DIR_.'product-verre.tpl');
			    ...
			    echo $url;
			//	exit;
				   Tools::redirect($url.'?id_product='.$id_product.'&type='.$_GET['type']);     
