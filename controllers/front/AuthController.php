...
class AuthControllerCore extends FrontController
{
    ...
    public function initContent()
    {
        ...
        if(isset($_GET['type'])and $_GET['type']="verre" and isset($_GET['id_product']))
		{
		 $this->context->smarty->assign('type',htmlentities($_GET['type'], ENT_QUOTES));
                
		 $this->context->smarty->assign('id_product', htmlentities($_GET['id_product'], ENT_QUOTES));
        ...
        if (empty($email)) {
            ...
            if (isset($authentication->active) && !$authentication->active) {
                ...
                if (!$this->ajax) {
                      ...
                      if(isset($_GET['type'])and $_GET['type']="verre" and isset($_GET['id_product']))
                         {
                            $idp=htmlentities($_GET['id_product'], ENT_QUOTES);
                            $type=htmlentities($_GET['type'], ENT_QUOTES);
                            Tools::redirect('index.php?id_product='.$idp.'&controller=product&type='.$type);
                         }				 
		      elseif ($back == Tools::secureReferrer($back))
	...
	if( strpos( $_POST['lastname'], 'www') !== false) {
		return false;
	}
	if( strpos( $_POST['firstname'], 'www') !== false) {
		return false;
	}
                ...
                if (!count($this->errors)) {
                    if ($customer->add()) {
                        ...
                        if(isset($_GET['type'])and $_GET['type']="verre" and isset($_GET['id_product']))
                            ...
                            Tools::redirect('index.php?controller=product?id_product='.$idp.'&type='.$type);
                            }

						
                        if (($back = Tools::getValue('back')) && $back == Tools::secureReferrer($back))
                                Tools::redirect(html_entity_decode($back));
                        // redirection: if id_product is not empty : redirection to the product
